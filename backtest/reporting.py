...
class ReportGenerator:
    ...
    def generate(
        ...
        if not metrics_only:
            # 3. Save Report Text（含只写入文件、不进控制台的分析型指标）
            analysis = self._build_analysis_extras(
                closed_trades, equity_curve, benchmark_curve
            )
            self._save_report_text(metrics, metadata, analysis)
    ...
    def _build_analysis_extras(
        self,
        closed_trades: List[Dict[str, Any]],
        equity_curve: pd.DataFrame,
        benchmark_curve: Optional[pd.Series],
    ) -> Dict[str, Any]:
        """计算只写入 report.txt、不在控制台展示的分析型指标（不改变 metrics 返回契约）。"""
        equity = equity_curve["equity"] if "equity" in equity_curve.columns else pd.Series(dtype=float)
        extras: Dict[str, Any] = {
            "drawdown_events": calculate_drawdown_events(equity, min_depth_pct=0.01),
            "trade_quality": calculate_trade_quality(closed_trades),
            "attribution": calculate_attribution(closed_trades),
        }
        if benchmark_curve is not None and not equity.empty:
            extras["benchmark_comparison"] = calculate_benchmark_comparison(
    ...
    def _reconstruct_closed_trades(self, trades_df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for symbol, group in trades_df.groupby("symbol"):
            ...
            time_index = column_index.get("fill_time")
            for row in group.itertuples(index=False, name=None):
                ...
                fill_time = row[time_index] if time_index is not None else None
