...
独立的 matplotlib 可视化脚本，读取某次回测报告的 `equity.csv`，画双联图（权益曲线 + 回撤百分比），输出 `performance_chart.png` 到同一报告目录。`plot_performance(report_dir)`、`get_latest_report_dir()`（不传 `--dir` 时按修改时间自动选最新的 `reports/*` 目录）。CLI：`--dir <path>`（可选）。只依赖 `ReportGenerator` 产出的 CSV，不直接导入 `core/`、`backtest/`、`live_trading/`——纯粹的报告消费者。
