# QuantTrading 2026-08-02 风险整改执行路线图

> 文档状态：Active v1.0  
> 基准日期：2026-08-02  
> 适用范围：当前工作区代码、测试、文档与运行产物  
...
本文将 2026-08-02 项目审查中发现的问题转化为可执行任务、依赖关系、验收标准和放行门槛。本文是阶段性风险整改路线图，不取代 `unified_roadmap.md` 的项目级权威地位，也不重复维护全项目完成状态。
...
unified_roadmap.md
  → development_plan.md
  → 2026_08_02_roadmap.md
  → 领域详细计划与当前行为文档
```

本文中的勾选表示本轮整改证据是否齐备；对应 R0–R8 阶段状态仍只在 `unified_roadmap.md` 更新。

## 2. 当前基线

### 2.1 已验证能力

- `python -m unittest discover -s tests -p "test_*.py" -v` 已运行；
- 共 60 个测试，全部通过；
- 已覆盖防前视、Next-Bar 执行、多标的事件时间线、部分成交、DAY/IOC/FOK、基础风险限制、固定回归样本和基础实盘同步 mock；
- 固定 fixtures 和三次结构一致性基线已经建立；
- 实盘引擎已有已收盘 bar 过滤、账户同步失败时停止交易、bar claim 和本地状态存储的初步实现。

### 2.2 当前阻断项

- 实盘默认不是 sandbox；
- API Key/Secret 仍可通过命令行传入；
- 下单异常被 `LiveBroker` 捕获后不向调用方传播，bar 仍可能被标记为已处理；
- `OrderStore`、订单状态机和交易所状态归一化尚未接入主链路；
- 未成交、部分成交和 unknown 订单可能被当作已执行记录；
- 账户同步没有完整表达 free、locked、open orders、positions 和交易所权益；
- 回测与实盘存在两套订单状态模型；
- 缺少重启恢复、周期性对账、故障退避、告警和稳定运行证据；
- Git 仍跟踪缓存、运行状态和部分历史报告；
- 当前工作区包含大量尚未提交的代码、文档和生成物变更。

## 3. 优先级定义

| 优先级 | 定义 | 处理要求 |
| --- | --- | --- |
| P0 | 可能造成重复订单、未知持仓、错误平仓、失效风控或资金错误 | 阻断真实资金，优先于所有策略和指标开发 |
| P1 | 可能造成回测失真、状态不可恢复、回测/实盘语义分叉或重大运维故障 | sandbox 长跑前完成 |
| P2 | 影响审计、监控、研究可信度、发布质量和可维护性 | 小额实盘评审前完成 |
| P3 | 策略、模型、性能、体验和非关键功能扩展 | 不得抢占 P0/P1 资源 |
...
- [ ] 保存当前 60 个测试的结构化结果和 Python/依赖版本；
...
## 11. G6：Sandbox 与 Paper Trading 长跑
...
## 14. G9：仓库、依赖、测试与 CI 治理

**映射阶段：R0/R6；优先级：P2。**

### 14.1 仓库卫生

- [ ] 在确认历史报告保留策略后，将已跟踪 `__pycache__` 和 `.pyc` 移出 Git 索引；
...
- [ ] 标记 production、research、archive、generated 和 deprecated 目录；
- [ ] 研究脚本复用正式回测执行与成本模型；
- [ ] 明确 `Trading_V1_Model.py` 和 `verify_*.py` 的保留或淘汰计划；
- [ ] ML 空壳模块要么实现最小契约，要么移入未来计划；
- [ ] Dashboard 提供明确入口和稳定输入 schema，或明确标记为未实现。
