...
class CircuitBreakerRecoveryFaultInjectionTests(unittest.TestCase):
    ...
    def test_portfolio_cooldown_keeps_exit_path_and_restart_deadline(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            engine = self._engine(store, risk)
            engine.event_processor._collect_symbol_candidate = MagicMock(return_value=(None, True))
            engine._tick()
            self.assertEqual(risk.breaker_action.value, "block_new")
            engine.event_processor._collect_symbol_candidate.assert_called_once()
            self.assertFalse(engine.event_processor._collect_symbol_candidate.call_args.kwargs["allow_new_entries"])
            self.assertTrue(engine.event_processor._collect_symbol_candidate.call_args.kwargs["allow_position_management"])
